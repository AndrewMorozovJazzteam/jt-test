(function ($) {
  ...
  $(document).ready(function () {
    ...
    function unifyHeight(selector, contentSelector = ".item-description", emptySelector = ".mediaholder.textholder") {
      let maxHeight = 0;
      let maxContentHeight = 0;

      $(selector).each(function () {
        const currentContentHeight = $(this).find(contentSelector).height();
      ...
      $(selector).each(function () {
        const empty = $(this).find(emptySelector);

        if (empty.length >= 1) {
          empty.addClass("strict--hidden");
          const content = $(this).find(contentSelector);
          const paddings = parseInt(content.css("padding-top")) + parseInt(content.css("padding-bottom"));
          console.log(paddings);
          content.css({ height: `${maxHeight - paddings}px` }).addClass("fixed--centered");
        } else {
          $(this)
            .find(contentSelector)
            .css({ height: `${maxContentHeight}px` })
            .addClass("fixed--centered");
        }
      });
    }
    ...
    function onSentinelTrigger(sentinel) {
      if (isMobile()) {
        const parentHeight = sentinel.parentElement.getBoundingClientRect().height;
        $(".columns.seven.pp_testimonials_block .overflowholder").css({ maxHeight: `${parentHeight + 16}px` });
      } else {
        $(".columns.seven.pp_testimonials_block .overflowholder").css({ maxHeight: `unset` });
      }
    }
    ...
    function initShowBizProSliders() {
      ...
      setTimeout(() => {
        ...
        unifyHeight(".columns.sixteen.pp_portfolio_block li .portfolio-item.media");
        unifyHeight(".columns.sixteen.pp_blog_block li .blog-item.media");

        const sentinels = injectSentinels(".columns.seven.pp_testimonials_block .testimonial");
