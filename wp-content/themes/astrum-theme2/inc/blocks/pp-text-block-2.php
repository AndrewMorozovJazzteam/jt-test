	...
	class PP_Text_Block2 extends AQ_Block {
		...
		function form($instance) {
			...
			<p class="description">
				...
				<label for="<?php echo $this->get_field_id('without_margins') ?>">
					<?php echo aq_field_checkbox('without_margins', $block_id, $without_margins) ?>
					<?php _e('Without margins', 'aqpb-l10n') ?>
				</label>
		...
		function block($instance) {
			...
			if($title) {
				if($without_margins == "0" || $without_margins == 0){
					echo '<div style="margin-top: 35px;"><'.$type.' data-toc-numbers="true" class="'.$classname.'" style="margin-top:0px;">'.strip_tags($title).'</'.$type.'><span class="line" style="margin-bottom:35px;"></span><div class="clearfix"></div>
</div>';
				}
				
				else{
					echo '<'.$type.' class="'.$classname.'" >'.strip_tags($title).'</'.$type.'><span class="line" style="margin-bottom:35px;"></span><div class="clearfix"></div>';
				}
